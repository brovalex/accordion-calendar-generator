...
function init() {
    ...
    const filename = './output/calendar-'+settings.template.name+'-'+settings.start.year+'-'+settings.start.month+'-'+(settings.week_starts_on==1?"monday":"sunday")+'-'+settings.paper.sizes.width+'x'+settings.paper.sizes.height+'('+Math.round(settings.paper.sizes.width*25.4*100)/100+'x'+Math.round(settings.paper.sizes.height*25.4*100)/100+')-'+settings.number_of_months+'months'+'.pdf' //'calendar.pdf' //
